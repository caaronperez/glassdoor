...
class NetworkManager {
  ...
  var rating: NetworkManagerRating?
  ...
  func createURLFromParameters(parameters: [String:Any]) -> URL {
    ...
    components.path = APIDetails.APIParamPath
  ...
  func downloadAPIInformation(parameters: [String: Any]){
          ...
          do {
            if let jsonArray = try JSONSerialization.jsonObject(with: data!, options: .allowFragments) as? [String: Any] {
              DispatchQueue.main.async {
                ...
                if let response = jsonArray[APIEndPoints.response]  {
                  self?.delegate?.didDownloadInformation(response: response as! [String: Any])
                } else {
    ...
    let task = URLSession.shared.dataTask(with: imgURL!) { [weak self] (data, response, error) in
      if error != nil {
        ...
        do  {
          if let imgData =  data{
            DispatchQueue.main.async {
              self?.rating?.didDownloadRating(image: UIImage(data: imgData)!)
            }
          }
        }
      }
